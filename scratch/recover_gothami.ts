...
async function main() {
  const schoolName = 'Co/Gothami Balika Vidyalaya';
  ...
  const membersData = [
    { registration_id: reg.id, name: 'D . D . Novanya Dewlini Alahakone', subject: 'physics', is_captain: true, access_code: 'PHY-MSW2' },
    { registration_id: reg.id, name: 'B .H . Pahanya Methsuwi Dharmasena', subject: 'maths', is_captain: false, access_code: 'MAT-VXNU' },
    { registration_id: reg.id, name: 'D . Sayuree Gimhani De Silva Wijayasinghe', subject: 'biology', is_captain: false, access_code: 'BIO-N2P2' },
    { registration_id: reg.id, name: 'B . Yesandi Vidunya Fernando', subject: 'chemistry', is_captain: false, access_code: 'CHE-PNB4' }
  ];
